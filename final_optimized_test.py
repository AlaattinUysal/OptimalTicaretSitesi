...
# --- TEST PARAMETRELERİ ---
MODEL_DOSYASI = "ppo_optimized_model.zip" # En son eğittiğimiz şampiyon model
HİSSE_KODU = "THYAO.IS"
BASLANGIC_BAKIYE = 100000

# Test için daha önce hiç görmediği bir tarih aralığı
TEST_BASLANGIC_TARIHI = "2025-01-01"
TEST_BITIS_TARIHI = "2025-09-08"  # Bugünün tarihi
...
if test_verisi is not None and len(test_verisi) > 15:
    ...
    print("\n--- NİHAİ OPTİMİZE EDİLMİŞ TEST SONUÇLARI ---")
    print(f"Başlangıç Bakiyesi: {BASLANGIC_BAKIYE:,.2f} TL")
    print("-" * 30)
    print(f"Optimize Edilmiş PPO Ajanının Portföy Sonucu: {ppo_sonuc:,.2f} TL")
    ...
    print(f"Optimize Edilmiş PPO Ajanı Getirisi: %{ppo_getiri:.2f}")
    print(f"'Al ve Tut' Getirisi: %{al_ve_tut_getiri:.2f}")

    if ppo_getiri > al_ve_tut_getiri:
        print("\nSonuç: Optimize edilmiş PPO ajanı, basit 'Al ve Tut' stratejisinden daha iyi bir performans gösterdi. ✅")
    else:
        print("\nSonuç: Optimize edilmiş PPO ajanı, basit 'Al ve Tut' stratejisini geçemedi. ❌")
    ...
    plt.title('Optimize Edilmiş Ajanın Test Verisi Üzerindeki Alım-Satım Noktaları')
    plt.xlabel('Tarih')
